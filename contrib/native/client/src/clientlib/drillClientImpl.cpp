        ...
        if(rmsgLen>0){
            ...
            *allocatedBuffer=currentBuffer;
            ...
            if(!error){
                ...
                Utils::freeBuffer(_buf, LEN_PREFIX_BUFLEN);
                DRILL_LOG(LOG_TRACE) << "DrillClientImpl::readMsg: ERR_QRY_COMMERR.\n";
            ...
            Utils::freeBuffer(_buf, LEN_PREFIX_BUFLEN);
            DRILL_LOG(LOG_TRACE) << "DrillClientImpl::readMsg: ERR_QRY_INVREADLEN.\n";
        ...
        if (qr->has_query_state()) {
            ...
            switch(qr->query_state()) {
                ...
                case exec::shared::QueryResult_QueryState_COMPLETED:
                    m_pendingRequests--;
                // Then fallback to the same control flow with the other state
                case exec::shared::QueryResult_QueryState_PENDING:
                case exec::shared::QueryResult_QueryState_RUNNING:
                    DRILL_LOG(LOG_DEBUG) << debugPrintQid(qid) << "Pending requests: "
                        << m_pendingRequests <<"." << std::endl;
...
status_t DrillClientImpl::processQueryId(AllocatedBufferPtr allocatedBuffer, InBoundRpcMessage& msg ){
    ...
    if(it!=this->m_queryIds.end()){
        ...
        delete allocatedBuffer;
        DRILL_LOG(LOG_TRACE) << "DrillClientImpl::processQueryId: ERR_QRY_INVQUERYID.\n";
...
void DrillClientImpl::handleRead(ByteBuf_t _buf,
    ...
    DRILL_LOG(LOG_TRACE) << "DrillClientImpl::handleRead: "
        << "Handle Read from buffer "
        <<  reinterpret_cast<int*>(_buf) << std::endl;
    if(DrillClientConfig::getQueryTimeout() > 0){
        // Cancel the timeout if handleRead is called
	DRILL_LOG(LOG_TRACE) << "DrillClient::handleRead: Cancel deadline timer"
	    << std::endl;
    ...
    if(!error){
        ...
        if(!error && msg.m_rpc_type==exec::user::QUERY_RESULT){
            ...
            if(error){
                ...
                //If not QUERY_RESULT, QUERY_HANDLE or HANDSHAKE then we think something serious has gone wrong?
				if(msg.m_rpc_type==exec::user::HANDSHAKE){
					if(msg.m_mode==exec::rpc::REQUEST){
